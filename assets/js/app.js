const lessonGroups = Array.isArray(window.builtInLessonGroups) ? window.builtInLessonGroups : [];
    ...
    const importedLessons = (Array.isArray(window.importedDocxLessons) ? window.importedDocxLessons : []).map(normalizeLessonRecord);
    ...
    const defaultStats = { attempts: 0, correct: 0, close: 0, streak: 0, seen: {}, weak: {} };
    state.stats = { ...defaultStats, ...state.stats, seen: state.stats.seen || {}, weak: state.stats.weak || {} };
    ...
    const contractions = new Map([
      ...
      ["couldn't", "could not"], ["won't", "will not"], ["wouldn't", "would not"], ["haven't", "have not"],
      ["hasn't", "has not"], ["hadn't", "had not"], ["that's", "that is"], ["what's", "what is"]
    ]);

    const articles = new Set(["a", "an", "the"]);
    const beForms = new Set(["am", "is", "are", "was", "were", "be", "been", "being"]);
    const prepositions = new Set(["in", "on", "at", "to", "for", "from", "with", "about", "of", "before", "after", "into", "by", "than"]);
    const auxiliaries = new Set(["do", "does", "did", "will", "would", "can", "could", "should", "must", "have", "has", "had"]);
    const weakVocabulary = new Set(["very", "good", "bad", "nice", "thing", "get", "make", "do"]);
    ...
    function seedUsers() {
      ...
      const merged = [...users];
      seeded.forEach(seed => {
        if (!merged.some(user => user.username === seed.username)) merged.push(seed);
      });
    ...
    function setCurrentUser(user) {
      if (user) {
        localStorage.setItem("englishTrainerCurrentUser", JSON.stringify({
      ...
      state.currentUser = user ? { username: user.username, plan: user.plan } : null;
    ...
    function customLessonsForActiveUser() {
      const username = activeUser().username;
      return customLessons.filter(lesson => !lesson.owner || lesson.owner === username);
    }
    ...
    function buildPremiumAnalysis(expectedText, actualText, lesson) {
      ...
      const details = [];
      if (missing.length) {
        details.push(`Phân tích PREMIUM: câu mẫu cần "${uniqueShort(missing, 4).join(", ")}" vì đây là phần mang nghĩa/cấu trúc chính của câu.`);
      }
      if (extra.length) {
        details.push(`Phân tích PREMIUM: "${uniqueShort(extra, 4).join(", ")}" có thể làm lệch cấu trúc so với câu chuẩn; hãy kiểm tra lại thì, giới từ và trật tự từ.`);
      }
      details.push(`Trọng tâm cần nhớ: ${lesson.focus || "diễn đạt tự nhiên trong tiếng Anh"}.`);
      return details.slice(0, 3);
    }

    function normalizeLessonRecord(record) {
      const english = Array.isArray(record.answers) ? record.answers.filter(Boolean) : [record.answer || record.text].filter(Boolean);
      return {
        id: String(record.id || `external-${Math.random().toString(36).slice(2)}`),
        level: ["A1", "A2", "B1", "B2"].includes(record.level) ? record.level : "A2",
    ...
    function loadCustomLessons() {
      return safeJsonParse(localStorage.getItem("englishTrainerCustomLessons"), []).map(normalizeLessonRecord);
    }

    function saveCustomLessons() {
      localStorage.setItem("englishTrainerCustomLessons", JSON.stringify(customLessons));
    }
    ...
    function refreshLessonCollections() {
      lessons = [...builtInLessons, ...importedLessons, ...customLessonsForActiveUser()];
    ...
    function normalize(text) {
      ...
      contractions.forEach((full, short) => {
        out = out.replace(new RegExp("\\b" + escapeRegExp(short) + "\\b", "g"), full);
      });
      return out
    ...
    function countMap(list) {
      return list.reduce((map, item) => map.set(item, (map.get(item) || 0) + 1), new Map());
    }
    ...
    function detectIssues(expectedText, actualText) {
      ...
      if (missingArticles.length) {
        notes.push(`Ngữ pháp: bạn đang thiếu mạo từ "${uniqueShort(missingArticles).join(", ")}". Trong câu mẫu, danh từ cần mạo từ để tự nhiên hơn.`);
      }

      const missingBe = missing.filter(word => beForms.has(word));
      if (missingBe.length) {
        notes.push(`Ngữ pháp: kiểm tra động từ "to be". Câu này cần "${uniqueShort(missingBe).join(", ")}".`);
      }

      const missingAux = missing.filter(word => auxiliaries.has(word));
      if (missingAux.length) {
        notes.push(`Ngữ pháp: bạn có thể đang thiếu trợ động từ "${uniqueShort(missingAux).join(", ")}", nên thì/câu hỏi/câu phủ định chưa đúng.`);
      }
      ...
      if (wrongPrep.length) {
        notes.push(`Từ vựng/giới từ: câu này hợp với "${uniqueShort(wrongPrep).join(", ")}". Gợi ý: giới từ trong tiếng Anh thường không dịch từng chữ từ tiếng Việt.`);
      }

      const expectedThirdPerson = expected.find((word, i) => i > 0 && ["he", "she", "it"].includes(expected[i - 1]) && word.endsWith("s") && !beForms.has(word));
      if (expectedThirdPerson && actual.includes(baseVerb(expectedThirdPerson)) && !actual.includes(expectedThirdPerson)) {
        notes.push(`Ngữ pháp: với "he/she/it" ở hiện tại đơn, động từ thường cần thêm "-s/-es": "${expectedThirdPerson}".`);
      }

      const expectedPast = expected.find(word => word.endsWith("ed") || ["met", "saw", "went", "left", "had", "was", "were", "decided", "asked"].includes(word));
      if (expectedPast && !actual.includes(expectedPast) && !actual.includes("did") && !actual.includes("had")) {
        notes.push(`Ngữ pháp: câu tiếng Việt có ý quá khứ, nên cần dạng quá khứ như "${expectedPast}".`);
      }

      const expectedPerfect = expected.includes("have") || expected.includes("has") || expected.includes("had");
      if (expectedPerfect && !actual.some(word => ["have", "has", "had"].includes(word))) {
        notes.push(`Ngữ pháp: câu này cần cấu trúc hoàn thành với "have/has/had" để diễn tả kinh nghiệm hoặc hành động xảy ra trước một mốc.`);
      }

      const shared = expected.filter(word => actual.includes(word));
      if (shared.length >= Math.min(5, expected.length - 1)) {
        const expectedSharedOrder = expected.filter(word => actual.includes(word)).join(" ");
        const actualSharedOrder = actual.filter(word => expected.includes(word)).join(" ");
        if (expectedSharedOrder !== actualSharedOrder) {
          notes.push("Trật tự từ: bạn có nhiều từ đúng, nhưng thứ tự trong câu chưa tự nhiên. Hãy đặt chủ ngữ trước động từ, sau đó đến tân ngữ/trạng ngữ.");
        }
      }

      const lexicalMissing = missing.filter(word => !articles.has(word) && !beForms.has(word) && !auxiliaries.has(word) && !prepositions.has(word));
      if (lexicalMissing.length) {
        notes.push(`Từ vựng: cần thêm hoặc thay bằng "${uniqueShort(lexicalMissing).join(", ")}" để sát nghĩa hơn.`);
      }

      const lexicalExtra = extra.filter(word => !articles.has(word) && !beForms.has(word) && !auxiliaries.has(word));
      if (lexicalExtra.length && notes.length < 5) {
        notes.push(`Từ không khớp: "${uniqueShort(lexicalExtra).join(", ")}" có thể làm câu lệch nghĩa hoặc thừa so với đáp án mẫu.`);
      }

      if (actual.some(word => weakVocabulary.has(word)) && expected.some(word => !actual.includes(word))) {
        notes.push("Từ vựng: thử dùng từ cụ thể hơn thay vì các từ rất chung như good/bad/thing/get nếu câu cần sắc thái rõ hơn.");
      }

      if (!notes.length) {
        notes.push("Câu của bạn gần đúng về ý. Hãy so sánh với đáp án mẫu để chỉnh lại cách diễn đạt, giới từ hoặc cụm từ cố định.");
      }
    ...
    function evaluateAnswer() {
      ...
      if (!answer) {
        showFeedback("wrong", "Nhập câu tiếng Anh trước đã.", 0, lesson.answers[0], ["Bạn chưa nhập câu nào. Hãy thử dịch trọn câu tiếng Việt ở trên."]);
        els.answerInput.focus();
        return;
      }

      const match = bestMatch(answer, lesson);
      const isCorrect = match.score >= 0.94 || lesson.answers.some(sample => normalize(sample) === normalize(answer));
      ...
      const notes = isCorrect
        ...
        : [...detectIssues(match.sample, answer), ...buildPremiumAnalysis(match.sample, answer, lesson)];
      ...
      showFeedback(isCorrect ? "correct" : isClose ? "close" : "wrong", title, Math.round(match.score * 100), match.sample, notes);
    ...
    function revealHint() {
      ...
      showFeedback("close", "Gợi ý", Math.round((hintSize / words.length) * 100), sample.replace(/[A-Za-z]/g, "_"), notes);
      els.answerInput.focus();
    }
    ...
    function filteredLessons() {
      ...
      if (state.level === "weak") {
        ...
        if (!items.length) items = planLessons.filter(item => item.level === "A1");
    ...
    function filteredListeningItems() {
      ...
      if (state.level === "weak") {
        ...
        if (!items.length) items = planListeningItems.filter(item => item.level === "A1");
      } else if (state.level !== "mix") {
    ...
    function startListening(useNext = false) {
      if (!("speechSynthesis" in window) || typeof SpeechSynthesisUtterance === "undefined") {
        showListeningFeedback("wrong", "Trình duyệt không hỗ trợ đọc", 0, "Không có âm thanh", [
      ...
      if (!state.listeningCurrent) {
        showListeningFeedback("close", "Không có câu nghe phù hợp", 0, "Hãy đổi bộ lọc", [
      ...
      els.listeningStatus.textContent = "Đang đọc đoạn tiếng Anh. Hãy nghe kỹ rồi nhập lại bên dưới.";
    ...
    function speakText(text) {
      ...
      utterance.onend = () => {
        els.listeningStatus.textContent = "Đã đọc xong. Nhập lại câu bạn nghe được rồi bấm kiểm tra.";
      };
    ...
    function nextListening() {
      ...
      state.listeningIndex = (state.listeningIndex + 1) % state.listeningQueue.length;
      state.listeningCurrent = currentListeningItem();
      state.listeningCheckedCurrent = false;
      startListening(true);
    }
    ...
    function checkListeningAnswer() {
      ...
      if (!state.listeningCheckedCurrent && !canPracticeMore()) {
        const plan = activePlan();
        showListeningFeedback("wrong", "Đã hết lượt hôm nay", 0, state.listeningCurrent.text, [
          `Tài khoản ${plan.name} chỉ được luyện tối đa ${plan.dailyLimit} câu/ngày.`,
          "Bạn có thể đăng nhập tài khoản PREMIUM hoặc quay lại vào ngày mai."
        ]);
        return;
      ...
      if (!answer) {
        showListeningFeedback("wrong", "Chưa nhập nội dung nghe", 0, state.listeningCurrent.text, [
      ...
      const isCorrect = score >= 0.94 || normalize(state.listeningCurrent.text) === normalize(answer);
      const isClose = !isCorrect && score >= 0.72;
      const notes = isCorrect
        ? ["Bạn nghe và chép lại rất tốt. Có thể bấm câu nghe tiếp để luyện thêm."]
        : [...detectIssues(state.listeningCurrent.text, answer), ...buildPremiumAnalysis(state.listeningCurrent.text, answer, state.listeningCurrent)];
      const title = isCorrect ? "Nghe đúng rồi" : isClose ? "Gần đúng" : "Cần nghe lại";
      ...
      if (!state.listeningCheckedCurrent) {
        ...
        if (isCorrect) {
          ...
          state.stats.streak = 0;
          state.stats.weak[state.listeningCurrent.id] = (state.stats.weak[state.listeningCurrent.id] || 0) + 1;
          if (isClose) state.stats.close += 1;
        }
      ...
      showListeningFeedback(isCorrect ? "correct" : isClose ? "close" : "wrong", title, Math.round(score * 100), state.listeningCurrent.text, notes);
      addHistory({
      ...
      }, answer, state.listeningCurrent.text, isCorrect, isClose);
      renderStats();
      renderProgress();
    }
    ...
    function renderLesson() {
      ...
      if (!lesson) {
        ...
        els.topicPill.textContent = state.topic === "all" ? "Tất cả chủ đề" : state.topic;
        els.indexPill.textContent = "0/0";
        els.vietnamesePrompt.textContent = "Không có câu phù hợp với bộ lọc hiện tại.";
        renderProgress();
        return;
      }
    ...
    function renderTopics() {
      const topics = [...new Set(lessons.filter(isAllowedByPlan).map(item => item.topic))].sort();
    ...
    function renderStats() {
      ...
      els.levelList.innerHTML = levels.map(level => {
        ...
        const total = lessons.filter(item => item.level === level && isAllowedByPlan(item)).length;
        const seen = lessons.filter(item => item.level === level && state.stats.seen[item.id]).length;
        return `
    ...
    function registerFreeAccount() {
      ...
      if (!username || !password) {
        els.accountStatus.textContent = "Nhập tên đăng nhập và mật khẩu để đăng ký.";
        return;
      }
    ...
    function refreshAfterAccountChange() {
      ...
      if (!activePlan().levels.includes(state.level) && state.level !== "mix" && state.level !== "weak") {
        state.level = "A1";
    ...
    function renderAccount() {
      ...
      const plan = activePlan();
      const usage = usageForActiveUser();
      const usageText = plan.dailyLimit === Infinity ? "Không giới hạn" : `${usage.practice}/${plan.dailyLimit} câu hôm nay`;
      const customText = plan.customLimit === Infinity ? "Không giới hạn câu tự thêm" : `${customLessonsForActiveUser().length}/${plan.customLimit} câu tự thêm`;
      ...
      document.querySelectorAll("[data-level]").forEach(button => {
        const locked = ["A1", "A2", "B1", "B2"].includes(button.dataset.level) && !plan.levels.includes(button.dataset.level);
    ...
    async function showUpgradeInfo() {
      const packages = paymentConfig.packages.map(pack => `${pack.label}: ${pack.price.toLocaleString("vi-VN")}đ`).join(" | ");
      const preview = await window.EnglishTrainerServices.createPayOSCheckout("premium_1m", activeUser());
      alert(`PREMIUM mở khóa B1-B2, luyện không giới hạn và phân tích lỗi nâng cao.\n\nGói: ${packages}\n\nThanh toán sau này: ${paymentConfig.method}. Code đã chuẩn bị cấu hình PayOS trong paymentConfig để nối backend tạo link/VietQR.`);
